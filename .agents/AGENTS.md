...
## 8. Safety when using AI agents on this project

- This is patient-data software. **Review every change before merge** — treat AI
  agents like a fast developer whose work is always reviewed, never an unreviewed committer.
...
## 9. DESIGN LANGUAGE (follow exactly — inspired by IndiGo airlines):
- Primary: deep indigo #131A8F, pressed/dark #0A1166, selected fill #E4E9FF
- Ink #23263B, secondary text #5B6172, hairlines #E3E8F4, white cards
- Background: soft sky wash gradient #F6FAFF → #DDEBFC
- Semantic accents (never decorative): cyan #5FC6E9 = info, orange #F08125 =
  attention, pink #ED2E7C = promo/highlight, green #1C9A4E = success
- Typography: 'Baloo 2' (rounded, 600–700) for headings and field values;
  'Nunito' for body/UI text
- Shape: cards 22px radius, fields 14px, ALL buttons/chips are pills (999px)
- Shadows indigo-tinted rgba(19,26,143,.08), never gray
- Signature widget pattern: bordered field-grid cells — tiny gray label on top,
  bold indigo value below (like an airline booking widget)
- Signature moment: confirmations render as an airline-style "MediPass"
  boarding pass with booking ref, token number, and perforated stub


  RULES: warm but clinical — legibility beats decoration; WCAG AA contrast;
no doctor-commission UI anywhere (locked off for India); statuses use the
semantic accent colors (Arrived=cyan, In consult=orange, Done=green).
